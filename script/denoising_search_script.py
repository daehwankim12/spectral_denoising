...
def main():
    ...
    parser.add_argument('-r', '--reference_lib_path', type=str, required=True,
                        help="The reference msp library file path")
    parser.add_argument('-o', '--output_directory', type=str,
                        help="The location of the output directory that msp files goes to")
    parser.add_argument('-p', '--peaks_col', type=str,
                        help="the column name of spectra column in reference file/ query file")
    parser.add_argument('-s', '--smiles_col', type=str,
                        help="the column name of smiles column in reference file/ query file")
    parser.add_argument('-a', '--adduct_col', type=str,
                        help="the column name of adduct column in reference file/ query file")
    parser.add_argument('-z', '--pmz_col', type=str,
                        help="the column name of precursor_mz column in reference file/ query file")
    parser.add_argument('-m', '--mass_tolerance', type=float, help="the mass tolerance for spectral denoising")
    parser.add_argument('-d', '--identity_search_mass_tolerance', type=float,
                        help="the mass tolerance for identity_search")
    # Parse the arguments
