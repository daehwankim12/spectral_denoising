...
def main():
    ...
    query_spectrum, query_smiles, query_adduct, query_pmz = query_data['peaks'], query_data['smiles'], query_data[
        'adduct'], query_data['precursor_mz']
    reference_spectrum = reference_data['peaks']
    denoised_spectrum = sd.spectral_denoising(query_spectrum, query_smiles, query_adduct)
    print(
        f'the denoised spectral has similarity of {entropy_similarity(denoised_spectrum, reference_spectrum, pmz=query_pmz):.2f}')
    ...
    print(
        f'the frist denoised spectra has similarity of {entropy_similarity(denoised_spectra[0], reference_spectrum, pmz=pmz):.2f}')
