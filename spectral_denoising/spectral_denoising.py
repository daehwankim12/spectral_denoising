...
def spectral_denoising_batch(msms_query, smiles_query, adduct_query, mass_tolerance=0.005):
    ...
    with mp.Pool(processes=6) as pool:
        ...
        results = pool.starmap(spectral_denoising, tqdm([(msms, smiles, adduct, mass_tolerance) for msms, smiles, adduct
                                                         in zip(msms_query, smiles_query, adduct_query)],
                                                        total=len(adduct_query)))
...
def spectral_denoising_with_master_formulas(msms, master_formula, benzene_tag, query_pmz, mass_tolerance=0.005):
    ...
    denoise_tag = get_denoise_tag(frag_msms, all_possible_candidate_formula, all_possible_mass, pmz, benzene_tag,
                                  mass_threshold)
...
def prep_formula(smiles, adduct):
    ...
    if is_smiles(smiles) == True:
        ...
        if adduct in ['[M]+', '[M]-'] and Chem.GetFormalCharge(mol) != 0:
            # print('i am in loop')
            if ((Chem.GetFormalCharge(mol)) > 0 and adduct[-1] == '+') or (
                    (Chem.GetFormalCharge(mol)) < 0 and adduct[-1] == '-'):
                # print('i am in loop')
                formula = formula[0:-1]
        ...
        elif (Formula(formula).charge == 1 and adduct == '[M]+') or (
                Formula(formula).charge == -1 and adduct == '[M]-'):
            formula = formula[0:-1]
...
def electronic_denoising(msms):
    ...
    msms_sorted = msms[np.argsort(msms[:, 1])].copy()

    confirmed_peaks_list = []

    while msms_sorted.shape[0] > 0:
        seed_intensity = msms_sorted[-1, 1]
        threshold = seed_intensity * 0.999

        idx_left = np.searchsorted(msms_sorted[:, 1], threshold, side='left')

        high_peaks = msms_sorted[idx_left:]

        if high_peaks.shape[0] <= 3:
            confirmed_peaks_list.append(high_peaks)

        msms_sorted = msms_sorted[:idx_left]

    if not confirmed_peaks_list:
        return np.nan

    final_peaks = np.concatenate(confirmed_peaks_list, axis=0)

    return so.sort_spectrum(so.pack_spectrum(final_peaks[:, 0], final_peaks[:, 1]))
...
def get_all_subformulas(raw_formula):
    ...
    all_possible_candidate_formula = [dict_to_formula(x, list(master_formula.keys())) for x in
                                      all_possible_candidate_formula_arr]
